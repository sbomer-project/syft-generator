...
Here is the complete, formatted README.md for the syft-generator component, incorporating the Architecture, Features, Configuration, and the new Development Setup workflow.
...
### 2. Driving Adapters (Input)
* **`KafkaRequestConsumer`:** Listens to the `generation.created` topic. If the request matches `sbomer.generator.name=syft`, it queues it for execution.
* **`TaskReconciler`:** A Kubernetes Controller (using Java Operator SDK) that watches for `TaskRun` completion. It updates the core domain when a task succeeds or fails.
...
### 3. Atomic Batch Uploads
The generated SBOMs are uploaded directly from the TaskRun pod to the [Manifest Storage Service](../manifest-storage-service) using an atomic batch transaction. The Generator Service receives the resulting URLs via the TaskRun results.

---

## Configuration

The service is configured via `application.properties` or Environment Variables.

| Property | Env Var | Description | Default |
| :--- | :--- | :--- | :--- |
| `sbomer.generator.name` | `SBOMER_GENERATOR_NAME` | The name used to filter incoming Kafka events. | `syft` |
| `sbomer.generator.syft.task-name` | `SBOMER_GENERATOR_SYFT_TASK_NAME` | The Tekton Task name to instantiate. | `generator-syft` |
| `sbomer.generator.max-concurrent` | `SBOMER_GENERATOR_MAX_CONCURRENT` | Max active TaskRuns allowed. | `20` |
| `sbomer.generator.oom-retries` | `SBOMER_GENERATOR_OOM_RETRIES` | Number of times to retry on OOM. | `3` |
| `sbomer.generator.memory-multiplier` | `SBOMER_GENERATOR_MEMORY_MULTIPLIER` | Factor to increase memory by on retry (e.g. 1.5x). | `1.5` |
| `sbomer.storage.url` | `SBOMER_STORAGE_URL` | internal URL of the storage service reachable by Pods. | `http://host.minikube.internal:8085` |
| `quarkus.kubernetes-client.namespace` | `QUARKUS_KUBERNETES_CLIENT_NAMESPACE` | The namespace where TaskRuns are created. | `default` |
